  ...
  return (
    <header className="flex justify-between gap-5 mt-10 w-full font-semibold max-md:flex-wrap max-md:mt-10 max-md:max-w-full">
      ...
      <nav className="flex self-end mt-8 text-sm text-zinc-500">
        <span className="text-emerald-400">Dashboard</span> / Data Master /
    ...
    <>
      <div className="flex w-full gap-0 mt-5 text-base text-center text-white max-md:flex-wrap">
        {dataHeadings.map((heading, index) => (
          <div
            key={index}
            className="justify-center px-4 py-3.5 whitespace-nowrap bg-teal-500 max-md:px-5"
          >
            {heading}
          </div>
        ))}
      ...
      {dataRows.map((row, index) => (
        <div
          key={index}
          className="flex w-full gap-0 text-base whitespace-nowrap text-neutral-600 max-md:flex-wrap"
        >
          <div className="z-10 justify-center px-10 py-7 bg-gray-200 max-md:px-5">
            {row.year}
          </div>
          <div className="justify-center px-12 py-7 bg-gray-200 max-md:px-5">
            {row.month}
          </div>
          <div className="justify-center px-10 py-7 text-center bg-gray-200 max-md:px-5">
            {row.province}
          </div>
          <div className="justify-center px-11 py-7 text-center bg-gray-200 max-md:px-5">
            {row.confirmed}
          </div>
          <div className="justify-center px-11 py-7 text-center bg-gray-200 max-md:px-5">
            {row.treated}
          ...
          <div className="justify-center px-5 py-6 text-center bg-gray-200 max-md:px-5">
            {row.treatment}
          </div>
        </div>
      ))}
  ...
  return (
    <div className="flex gap-5 justify-between w-full max-md:flex-wrap max-md:max-w-full">
      ...
      <input
        type="text"
        placeholder="Search ..."
        className="justify-center items-start px-2.5 py-3 text-base bg-white rounded-md border-black border-solid text-zinc-600 max-md:pr-5"
      />
    </div>
  );
}
